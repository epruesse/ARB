...
void GEN_add_organism_to_hash(GBDATA *gb_organism, GB_HASH *organism_hash) {
    ...
    GBS_write_hash(organism_hash, name, (long)gb_organism);
}

GB_HASH *GEN_create_organism_hash(GBDATA *gb_main) {
    GB_HASH *organism_hash = GBS_create_hash(GBT_get_species_hash_size(gb_main), 1);
    GBDATA  *gb_organism;

    for (gb_organism = GEN_first_organism(gb_main);
         gb_organism;
         gb_organism = GEN_next_organism(gb_organism))
    {
        GEN_add_organism_to_hash(gb_organism, organism_hash);
    }
...
GBDATA *GEN_find_origin_organism(GBDATA *gb_pseudo, GB_HASH *organism_hash) {
    ...
    // otherwise it shall be a hash generated by GEN_create_organism_hash() [if you search several times]
    // Note : use GEN_add_organism_to_hash() to keep hash up-to-date
