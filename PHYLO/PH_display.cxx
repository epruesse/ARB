...
void ph_view_filter_cb(AW_window *aww,AW_CL cb1,AW_CL cb2)
{
    AWUSE(cb1); AWUSE(cb2);
 AW_window *main_win = PH_used_windows::windowList->phylo_main_window;
 AWUSE(aww);
...
void AP_display::resized(void)
{
 ...
 PH_used_windows::windowList->phylo_main_window->get_device(AW_MIDDLE_AREA)->
     get_area_size(&squ);
 ...
 {
     case NONE: return;
     case species_dpy: horiz_paint_size=(squ.r-off_dx)/cell_width;
	 vert_paint_size=(squ.b-off_dy)/cell_height;
	 horiz_page_size = (PHDATA::ROOT->get_seq_len() > horiz_paint_size) ?
	     horiz_paint_size : PHDATA::ROOT->get_seq_len();
	 vert_page_size = (long(PHDATA::ROOT->nentries) > vert_paint_size) ?
	     vert_paint_size : PHDATA::ROOT->nentries;
	 rect.l=0;
	 rect.t=0;
	 rect.r = (int)
	     ((PHDATA::ROOT->get_seq_len()-horiz_page_size)*cell_width+squ.r);
	 rect.b = (int)
	     ((PHDATA::ROOT->nentries-vert_page_size)*cell_height+squ.b);
	 break;            
     case matrix_dpy: horiz_paint_size=(squ.r-aw_fi->max_letter_width-off_dx)/cell_width;
	 vert_paint_size=(squ.b-off_dy)/cell_height;
	 horiz_page_size = (long(PHDATA::ROOT->nentries) > horiz_paint_size) ?
	     horiz_paint_size : PHDATA::ROOT->nentries;
	 vert_page_size = (long(PHDATA::ROOT->nentries) > vert_paint_size) ?
	     vert_paint_size : PHDATA::ROOT->nentries;
	 rect.l=0;
	 rect.t=0;
	 rect.r = (int)
	     ((PHDATA::ROOT->nentries-horiz_page_size)*cell_width+squ.r);  
	 rect.b = (int)
	     ((PHDATA::ROOT->nentries-vert_page_size)*cell_height+squ.b);
	 break; 
     case filter_dpy: horiz_paint_size=(squ.r-off_dx)/cell_width;
	 vert_paint_size=(squ.b-off_dy)/cell_height;
	 ...
	 rect.b = (int)
	     ((PHDATA::ROOT->nentries-vert_page_size)*cell_height+squ.b);
	 break;
     default: aw_message("resized: unknown display type (maybe not implemented yet)");
 }  // switch
...
void display_status(AW_window *dummy,AW_CL awroot,AW_CL cd2)    // bottom area
{
    ...
    AW_root *aw_root=(AW_root *) awroot; 
