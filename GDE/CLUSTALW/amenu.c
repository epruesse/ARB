...
static sint read_matrix(char **mattxt, char *matnam, sint matn, short *mat, short *xref);
static sint read_dnamatrix(char **mattxt, char *matnam, sint matn, short *mat, short *xref);
...
extern char *matrix_txt[];
extern char *pw_matrix_txt[];
extern char *dnamatrix_txt[];
	...
	fprintf(stdout,"    6.  Output format options\n");
        ...
        switch(toupper(*lin1))
        {
        	...
        	case '2': tossgaps ^= TRUE;
      		...
      		case '3': kimura ^= TRUE;;
            		break;
        	case '4': phylogenetic_tree(phylip_name,clustal_name,dist_name);
            		break;
        	case '5': bootstrap_tree(phylip_name,clustal_name);
            		break;
...
{	
	...
	fprintf(stdout,"     4. Toggle Phylip bootstrap positions    =  %s\n\n",
...
{	
	...
	fprintf(stdout,"     7. Toggle CLUSTALW sequence numbers =  %s\n",
					(!cl_seq_numbers) ? "OFF" : "ON");
	fprintf(stdout,"     8. Toggle output order              =  %s\n\n",
					(output_order==0) ? "INPUT FILE" : "ALIGNED");
	fprintf(stdout,"     9. Create alignment output file(s) now?\n\n");
	fprintf(stdout,"     0. Toggle parameter output          = %s\n",
			...
			case '2':
				...
				break;
                        case '3':
                                ...
                                break;
                        case '4':
                                pw_dnamatnum = read_dnamatrix(dnamatrix_txt,pw_dnamtrxname,pw_dnamatnum,pw_userdnamat,pw_dna_xref);
                                break;
			case '5':
	...
	while(TRUE) {
		...
		switch(toupper(*lin2)) {
			...
			case '5':
				...
				break;
			case '6':
                                dnamatnum = read_dnamatrix(dnamatrix_txt,dnamtrxname,dnamatnum,userdnamat,dna_xref);
				break;
			case '7':
				neg_matrix ^= TRUE;
				break;
...
{       static char userfile[FILENAMELEN+1];
        ...
        {
                fprintf(stdout,"\n\n\n");
                fprintf(stdout," ********* DNA WEIGHT MATRIX MENU *********\n");
                fprintf(stdout,"\n\n");
