...
/*
 * version 3.56c. (c) Copyright 1993 by Joseph Felsenstein. Written by Joseph
 * Felsenstein, Hisashi Horino, Akiko Fuseki, Sean Lamont, and Andrew Keeffe.
...
Static boolean  anerror, trout, first, noroot, outgropt, didreroot, prntsets, progress,
                treeprint, ibmpc, vt52, ansi, goteof;
...
uppercase(ch)
	Char           *ch;
{				/* convert ch to upper case -- either ASCII
				 * or EBCDIC */
...
void 
addelement(p, q, ch, parens, nextnode)
	node          **p, *q;
	Char           *ch;
	...
	node           *p;
{
	...
	fprintf(treefile, ":%1.3f", x/(double)ntrees);
	col += 4;
}				/* treeout */
