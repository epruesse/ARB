...
AW_window *st_create_main_window(AW_root *root, ST_ML *st_ml, AW_CB0 refresh_func, AW_window *win){
	...
	aws->init( root, "ENABLE_ONLINE_STATISTIC", "ACTIVATE ONLINE STATISTIC", 100, 100 );
...
ST_ML_Color *st_ml_get_color_string(ST_ML *st_ml, char *species_name, AP_tree *node, int start_ali_pos, int end_ali_pos) {
    return st_ml->get_color_string(species_name,node,start_ali_pos, end_ali_pos);
}

int st_ml_update_ml_likelihood(ST_ML *st_ml, char *result[4], int *latest_update, char *species_name, AP_tree *node) {
    return st_ml->update_ml_likelihood(result, latest_update, species_name, node);
}

AP_tree *st_ml_convert_species_name_to_node(ST_ML *st_ml,const char *species_name){
    AP_tree *node;
...
AW_window *st_create_quality_check_window(AW_root *root,GBDATA *gb_main){
    ...
    aws->init( root, "SEQUENCE_QUALITY_CHECK", "CHECK QUALITY OF MARKED SEQUENCES", 100, 100 );
...
//    aws->load_xfig("stat_main.fig");
	...
	awt_create_selection_list_on_scandb(gb_main,aws,ST_ML_AWAR_CQ_DEST_FIELD, 1 << GB_STRING, "dest",0, &AWT_species_selector, 20, 10);
	aws->at("GO");
