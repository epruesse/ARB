...
static sint check_param(char *inline1,char *params[], char *param_arg[]);
...
*/
	...
	command_sep[0] = COMMANDSEP;
	command_sep[1] = '\0';

	if (inline1[0] != COMMANDSEP) name1 = TRUE;

        inptr=inline1;
        for (i=0;i<MAXARGS;i++) {
                if ((params[i]=strtok(inptr,command_sep))==NULL)
                        break;
                inptr=NULL;
        }
        if (i==MAXARGS) {
...
/*** ? /pwmatrix=ID (user's file)  */
	if(setpwmatrix != -1)
		if(strlen(param_arg[setpwmatrix]) > 0) {
                        ...
                        else if (strcmp(param_arg[setpwmatrix],"gonnet")==0) {
                                strcpy(pw_mtrxname, param_arg[setpwmatrix]);
                                pw_matnum = 3;
                        }
...
/*** ? /matrix=ID (user's file)  */
	if(setmatrix != -1)
		if(strlen(param_arg[setmatrix]) > 0) {
                        ...
                        else if (strcmp(param_arg[setmatrix],"gonnet")==0) {
                                strcpy(mtrxname, param_arg[setmatrix]);
                                matnum = 3;
                        }
...
/*** ? /pwdnamatrix=ID (user's file)  */
	if(setpwdnamatrix != -1)
		if(strlen(param_arg[setpwdnamatrix]) > 0) {
                        if (strcmp(param_arg[setpwdnamatrix],"iub")==0) {
                                strcpy(pw_dnamtrxname, param_arg[setpwdnamatrix]);
                                pw_dnamatnum = 1;
                        }
                        else if (strcmp(param_arg[setpwdnamatrix],"clustalw")==0) {
                                strcpy(pw_dnamtrxname, param_arg[setpwdnamatrix]);
                                pw_dnamatnum = 2;
                        }
			else {
...
/*** ? /matrix=ID (user's file)  */
	if(setdnamatrix != -1)
		if(strlen(param_arg[setdnamatrix]) > 0) {
                        if (strcmp(param_arg[setdnamatrix],"iub")==0) {
                                strcpy(dnamtrxname, param_arg[setdnamatrix]);
                                dnamatnum = 1;
                        }
                        else if (strcmp(param_arg[setdnamatrix],"clustalw")==0) {
                                strcpy(dnamtrxname, param_arg[setdnamatrix]);
                                dnamatnum = 2;
                        }
			else {
...
/****************************************************************************/
	...
	if(do_profile) {
		...
		phylogenetic_tree(phylip_tree_name,clustal_tree_name,dist_tree_name);

	else if(do_boot)
		bootstrap_tree(phylip_tree_name,clustal_tree_name);
...
*/
	...
	if(reset_alignments) reset_align();
...
void profile_align(char *p1_tree_name,char *p2_tree_name)
{
	...
	if (use_tree2 == FALSE)
		if(nseqs-profile1_nseqs > 3) {
                	for (i=1;i<=nseqs-profile1_nseqs;i++) {
                        	for (j=i+1;j<=nseqs-profile1_nseqs;j++) {
                                	dscore = countid(i+profile1_nseqs,j+profile1_nseqs);
                                	tmat[i][j] = (100.0 - dscore)/100.0;
                                	tmat[j][i] = tmat[i][j];
                        	}
                	}
...
/* decide the line length for this alignment - maximum is LINELENGTH */
	...
	for(lv1=1;lv1<=chunks;++lv1) {
		...
		for(i=pos;i<=ptr;++i) {
			...
			for(j=fseq;j<=lseq;++j) {
				if((seq_array[fseq][i] >=0) && 
				   (seq_array[fseq][i] <= max_aa)) {
...
static void reset_align(void)   /* remove gaps from older alignments (code = gap_pos1) */
...
static void reset_prf1(void)   /* remove gaps from older alignments (code = gap_pos1) */
{		      				/* EXCEPT for gaps that were INPUT with the seqs.*/
	register sint sl;   		     /* which have  code = gap_pos2  */
	sint i,j;
	...
	if (struct_penalties1 != NONE) {
		...
		for (j=0;j<seqlen_array[1];++j) {
			if (gap_penalty_mask1[j] == gap_pos1) continue;
	...
	if (struct_penalties1 == SECST) {
		sl=0;
		for (j=0;j<seqlen_array[1];++j) {
			if (sec_struct_mask1[j] == gap_pos1) continue;
...
static void reset_prf2(void)   /* remove gaps from older alignments (code = gap_pos1) */
{		      				/* EXCEPT for gaps that were INPUT with the seqs.*/
	register sint sl;   		     /* which have  code = gap_pos2  */
	sint i,j;
	...
	if (struct_penalties2 != NONE) {
		...
		for (j=0;j<seqlen_array[profile1_nseqs+1];++j) {
			if (gap_penalty_mask2[j] == gap_pos1) continue;
	...
	if (struct_penalties2 == SECST) {
		sl=0;
		for (j=0;j<seqlen_array[profile1_nseqs+1];++j) {
			if (sec_struct_mask2[j] == gap_pos1) continue;
