...
void gb_load_single_key_data(GBDATA *gb_main,GBQUARK	q){
    ...
    const char *key = Main->keys[q].key;
    ...
    if (key[0] == '@'){
        ...
        if (gb_dict){
            if (Main->keys[q].dictionary){
                delete_gb_dictionary(Main->keys[q].dictionary);
            }
            ks->dictionary = gb_create_dict(gb_dict);
        }else{
            if (Main->keys[q].dictionary){
                GB_internal_error("Uups Dictionary deleted");
            }
            ks->dictionary = 0;
        }
        ks->gb_key = gb_key;
...
GB_ERROR gb_save_dictionary(GBDATA *gb_main,const char *key,const char *dict, int size){
    GB_MAIN_TYPE *Main = GB_MAIN(gb_main);
    GB_ERROR error = 0;
    gb_main = (GBDATA *)Main->data;
    if (key[0] == '@'){
        return GB_export_error("No dictionaries for system fields");
    }else{
        GBDATA *gb_key_data = Main->gb_key_data;
        GBDATA *gb_key,*gb_name,*gb_dict;
        GB_push_my_security(gb_main);
