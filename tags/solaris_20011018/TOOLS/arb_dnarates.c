...
void getbasefreqs ()
  { /* getbasefreqs */
    ...
    printf("Transition/transversion ratio = %10.6lf\n\n", ttratio);
    ...
    printf("(Transition/transversion parameter = %10.6lf)\n\n", xi/xv);
...
void setuptree (tr, numsp)
  ...
  { /* setuptree */
    ...
    for (i = numsp+1; i <= 2*numsp-2 && ! anerror; i++) { /* Internal nodes */
      q = (node *) NULL;
      for (j = 1; j <= 3; j++) {
        if (anerror = !(p = (nodeptr) malloc((unsigned) sizeof(node))))
          break;
        p->x      = (xarray *) NULL;
        p->tip    = (char *) NULL;
        p->number = i;
...
void getdata (tr)
  ...
  { /* getdata */
    ...
    for (j = 1; j <= numsp; j++)
      for (i = 1; i <= sites; i++)
        if ((y[j][i] = meaning[y[j][i]]) != 15)  info[i]++;
