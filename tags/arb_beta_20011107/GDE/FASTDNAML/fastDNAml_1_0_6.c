...
#define programDate     "November 20, 1992"
 ...
 *
 *  1.0.5  Fix reading of underscore as first nonblank character in name
 *         Add strchr and strstr functions to source code
 *         Add output treefile name to message "Tree also written ..."
 *
 *  1.0.6  Change (! nsites) test in setupTopol to (nsites == 0) for MIPS R4000
...
#   ifdef Vectorize
       double   zz[maxsites],zv[maxsites];
#   else
       double   zz,zv;
#   endif
...
void sendTree (comm, tr)
    ...
    tree *tr;
  { /* sendTree */
    char treestr[maxsp*(nmlngth+24)+100];
    char *treeString();
#   if Master
      void sendTreeNum();
#   endif
