...
ED4_returncode ED4_bracket_terminal::draw() {
    ...
    if (multi_man->get_no_of_selected_species()) {  // if multi_species_manager contains selected species
        ...
        toggle             = !toggle;
        device->box(toggle ? ED4_G_SELECTED : ED4_G_SELECTED+1, AW::FillStyle::FILLED, term_area);
#else // !defined(DEBUG)
        device->box(ED4_G_SELECTED, AW::FillStyle::FILLED, term_area);
#endif
    }
...
#if defined(DEBUG)
    device->box(ED4_G_MARKED, AW::FillStyle::FILLED, x1, y1+1, x2-x1+1, y2-y1-1);
#else
        ...
        for (int i=left; i<=right; i++, x2+=font_width) { // colorize significant columns in ALL rows
            ...
            if (color!=old_color) {
                if (x2>old_x2 && old_color!=ED4_G_STANDARD) {
                    device->box(old_color, AW::FillStyle::FILLED, old_x2, y, x2-old_x2, term_height);
                }
                old_color = color;
                old_x2 = x2;
            }
        ...
        for (int i=left; i<=right; i++, x2+=font_width) { // colorize significant columns in SINGLE rows
            ...
            if (found && significance<100) {
                ...
                if (color!=ED4_G_STANDARD) {
                    ...
                    {
                        if (found&bit) {
                            device->box(color, AW::FillStyle::FILLED, x2, y2-2*font_height+1, font_width, 2*font_height);
                        }
                    }
                }
            }
