...
//     return 1;
// }
// void show_primes(int maxval) {
//     printf("primes between 3 and %i: ", maxval);
//     for (int m=3; m<=maxval; m+=2) {
// 	if (is_prime(m)) {
// 	    printf("%i ", m);
// 	}
//     }
...
// #if 0
// #define WORLD_COORDS_CACHE_SIZE 2237 // size of hash table used to cache world-coords (should be a prime number)

// static int WCC_actualTimeStamp = 1;
// static int WCC_recentUsedTimeStamp = 1;

// static const ED4_base *WCC_base[WORLD_COORDS_CACHE_SIZE];
// static AW_pos WCC_xpos[WORLD_COORDS_CACHE_SIZE];
// static AW_pos WCC_ypos[WORLD_COORDS_CACHE_SIZE];
// static int WCC_timeStamp[WORLD_COORDS_CACHE_SIZE];

// static int hit = 0;
// static int miss = 0;

// static inline int calc_WCC_hashValue(const ED4_base *b) {
//     return int(b)%WORLD_COORDS_CACHE_SIZE;
// }

// void ED4_base::touch_world_cache() {
//     if (WCC_actualTimeStamp==WCC_recentUsedTimeStamp) {
// 	WCC_actualTimeStamp++;
//     }
// }

// void ED4_base::calc_world_coords(AW_pos *x, AW_pos *y) const { // calculates world coordinates of current object by adding up the relative offsets
//     int idx = calc_WCC_hashValue(this);

//     if (WCC_base[idx]==this && WCC_timeStamp[idx]==WCC_actualTimeStamp) {
// 	*x = WCC_xpos[idx];
// 	*y = WCC_ypos[idx];
// 	hit++;
//     }
...
// 	miss++;
// 	if (parent) {
// 	    parent->calc_world_coords(x, y);
// 	}
// 	else {
// 	    *x = extension.position[X_POS];
// 	    *y = extension.position[Y_POS];
// 	}

// 	WCC_base[idx] = this;
// 	WCC_xpos[idx] = *x += extension.position[X_POS];
// 	WCC_ypos[idx] = *y += extension.position[Y_POS];
// 	WCC_timeStamp[idx] = WCC_recentUsedTimeStamp = WCC_actualTimeStamp;
//     }

// #ifdef DEBUG
//     if (hit+miss>=10000) {
// 	printf("World-coords cache hit-rate = %f\n", float(hit/10000.0));
// 	hit = miss = 0;
//     }
// #endif
// }
...
// #if 0
// void ED4_base::calc_world_coords(AW_pos *xPtr, AW_pos *yPtr) const // calculates world coordinates of current object by adding up the relative offsets
// {
//     ED4_base *temp = this;
//     AW_pos x = 0;
//     AW_pos y = 0;
...
//     while (temp) {
// 	x += temp->extension.pos[X_POS];
// 	y += temp->extension.pos[Y_POS];
// 	temp = temp->parent;
//     }
