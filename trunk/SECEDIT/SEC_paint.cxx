...
//     }
  ...
  // set_root_segment(segment_pointer); //setting the root segment
...
void SEC_helix_strand::paint_strands(AW_device *device, double *v, double &length_of_v){
    ...
    //double fontWidth    = transform_size(device, font_info[SEC_GC_HELIX].get_ascent())/2.0;
    device->set_line_attributes(SEC_GC_HELIX,3, AW_SOLID);  //setting the line attributes
    double font_size;
    ...
    for (int i=0,j=(max_base_count-1); i<max_base_count; i++,j--) {
		...
		device->line(SEC_GC_NHELIX, thisLast_x, thisLast_y-font_height2, this_x, this_y-font_height2, -1, 0, 0);
		thisLonelyBaseClrTag = 0;
	      }
	      else {
		if(root->show_strSkeleton) device->line(SEC_GC_HELIX, thisLast_x, thisLast_y-font_height2, this_x, this_y-font_height2, -1, 0, 0);
	      }
		...
		device->line(SEC_GC_NHELIX, otherLast_x, otherLast_y-font_height2, other_x, other_y-font_height2, -1, 0, 0);
		otherLonelyBaseClrTag = 0;
	      }
	      else {
		if(root->show_strSkeleton) device->line(SEC_GC_HELIX, otherLast_x, otherLast_y-font_height2, other_x, other_y-font_height2, -1, 0, 0);
	      }
...
void SEC_helix_strand::print_lonely_bases(char *buffer, AW_device *device, double attachpA_x, double attachpA_y, double attachpB_x, double attachpB_y, double base_x, double base_y,
    ...
    device->set_line_attributes(SEC_GC_NHELIX,3, AW_SOLID);  //setting the line attributes

    if(thisStrand){
	if(root->show_strSkeleton) device->line(SEC_GC_NHELIX, thisLast_x, thisLast_y-half_font_height, print_pos_x, print_pos_y-half_font_height, -1, 0, 0);
      ...
      if(root->show_strSkeleton) device->line(SEC_GC_NHELIX, otherLast_x, otherLast_y-half_font_height, print_pos_x, print_pos_y-half_font_height, -1, 0, 0);
...
void SEC_segment::paint(AW_device *device, SEC_helix_strand *previous_strand_pointer) {
	...
	device->set_line_attributes(SEC_GC_LOOP,3, AW_SOLID);  //setting the line attributes
	if(i==0  && root->show_strSkeleton)  device->line(SEC_GC_LOOP, start_x, start_y-font_height2,next_x, next_y-font_height2, -1, 0, 0);
    ...
    if(root->show_strSkeleton) device->line(SEC_GC_LOOP, next_x, next_y-font_height2, nextCircle_x, nextCircle_y-font_height2, -1, 0, 0);
