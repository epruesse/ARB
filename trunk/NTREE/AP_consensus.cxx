...
void CON_calculate_cb(AW_window *aw )
{
	...
	if(strcmp("marked",marked)) onlymarked=0;
	...
	if(lower>fupper) {
		aw_message("fault: lower greater than upper");
		aw_closestatus();
		delete align;
		delete marked;
		delete complexresult;
	...
	int countgap=1;
	if(strcmp("on",countgaps)) countgap=0;

    /* creating the table for groups */
	char *groupflags[40], groupnames[40];
	int numgroups;
	numgroups=CON_makegrouptable(groupflags,groupnames,isamino,groupallowed);

    /* calculate and export the result strings */
	char *result=0;
	CON_evaluatestatistic(result,statistic,groupflags,groupnames,
                          (int)maxalignlen,fupper,lower,fconsidbound,gapbound,countgap,numgroups);

	char *savename = awr->awar("tmp/con/name")->read_string();
	GB_ERROR error=CON_export(savename,align,statistic,result,convtable,groupnames,
