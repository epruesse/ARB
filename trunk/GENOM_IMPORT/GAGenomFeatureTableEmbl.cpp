...
gellisary::GAGenomFeatureTableEmbl::GAGenomFeatureTableEmbl()
{
...
int gellisary::GAGenomFeatureTableEmbl::nameToNumberOfFeature(string * source_str)
{
    for(int i = 0; i < (int)features.size();i++)
    {
...
void gellisary::GAGenomFeatureTableEmbl::parse()
{
    string tmp_str;
    string t_str;
    bool newgene = true;
...
GAGenomFeatureTableSourceEmbl * gellisary::GAGenomFeatureTableEmbl::getFeatureTableSource()
{
    if(!prepared)
    {
        parse();
...
string * gellisary::GAGenomFeatureTableEmbl::getGeneName()
{
    if(!prepared)
    {
        parse();
