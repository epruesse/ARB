...
nt_mode_event( AW_window *aws, AWT_canvas *ntw, AWT_COMMAND_MODE mode)
{
	...
	case AWT_MODE_MOVE:
	    ...
	    break;
	case AWT_MODE_NNI:
	    text="NEAREST NEIGHBOUR INTERCHANGE OPTIMIZER  L: select subtree M:whole tree";
	    break;
	case AWT_MODE_KERNINGHAN:
	    text="KERNIGHAN LIN OPTIMIZER   L: select subtree M:whole tree";
	    break;
	case AWT_MODE_OPTIMIZE:
	    text="NNI & KL OPTIMIZER   L: select subtree M:whole tree";
	    break;
...
void
NT_mark_all_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
...
void
NT_unmark_all_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
    GB_push_transaction(ntw->gb_main);
...
void
NT_invert_mark_all_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
    GB_push_transaction(ntw->gb_main);
    GBDATA *gb_species;
...
void
NT_count_mark_all_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
    GB_push_transaction(ntw->gb_main);
    long count=0;
...
void
NT_mark_tree_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
...
void
NT_group_terminal_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
    GB_transaction gb_dummy(ntw->gb_main);
    AWT_TREE(ntw)->check_update(ntw->gb_main);
...
void
NT_resort_tree_cb(void *dummy, AWT_canvas *ntw,int type)
{
    AWUSE(dummy);
    GB_transaction gb_dummy(ntw->gb_main);
    AWT_TREE(ntw)->check_update(ntw->gb_main);
...
void
NT_ungroup_all_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
    GB_transaction gb_dummy(ntw->gb_main);
    AWT_TREE(ntw)->check_update(ntw->gb_main);
...
void
NT_reset_lzoom_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
    GB_transaction gb_dummy(ntw->gb_main);
    AWT_TREE(ntw)->check_update(ntw->gb_main);
...
void
NT_reset_pzoom_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
    GB_transaction gb_dummy(ntw->gb_main);
    AWT_TREE(ntw)->check_update(ntw->gb_main);
...
void
NT_unmark_all_tree_cb(void *dummy, AWT_canvas *ntw)
{
    AWUSE(dummy);
    GB_transaction gb_dummy(ntw->gb_main);
    AWT_TREE(ntw)->check_update(ntw->gb_main);
...
void
NT_set_tree_style(void *dummy, AWT_canvas *ntw, AP_tree_sort type)
{
    AWUSE(dummy);
    GB_transaction gb_dummy(ntw->gb_main);
    AWT_TREE(ntw)->check_update(ntw->gb_main);
...
void
NT_remove_bootstrap(void *dummy, AWT_canvas *ntw) // delete all bootstrap values
{
    AWUSE(dummy);
    GB_transaction gb_dummy(ntw->gb_main);
    AWT_TREE(ntw)->check_update(ntw->gb_main);
...
void 
NT_jump_cb(AW_window *dummy, AWT_canvas *ntw, AW_CL auto_expand_groups)
{
    AWUSE(dummy);
...
void 
NT_jump_cb_auto(AW_window *dummy, AWT_canvas *ntw){	// jump only if auto jump is set
