...
#define T(a,b,c)	awt_pro_a_nucs_build_table(a,b,c)
	...
	// order  of table is important: mussed be compatible with DIST/PH_protdist.cxx !!
...
void awt_pro_a_nucs_gen_dist(GBDATA *gb_main){
	...
	for (s = 0; s< awt_pro_a_nucs->max_aa; s++){
		...
		for (i=0;i<3;i++) {
			awt_pro_a_nucs->dist[s]->nucbits[i] =
			awt_pro_a_nucs->s2str[awt_pro_a_nucs->index_2_spro[s]]->nucs->nucbits[i];
		}
	}

	for (s = 0; s< awt_pro_a_nucs->max_aa; s++){
		for (d = 0; d< awt_pro_a_nucs->realmax_aa; d++){
			dist = AP_nuc_dist( 	awt_pro_a_nucs->index_2_spro[s],
						awt_pro_a_nucs->index_2_spro[d] );
			if (dist==0) {
				awt_pro_a_nucs->dist[s]->patd[0] |= 1<<d;
			}

			if (dist<=1){
				awt_pro_a_nucs->dist[s]->patd[1] |= 1<<d;
			}
		}
		awt_pro_a_nucs->dist[s]->patd[2] |= awt_pro_a_nucs->dist[s]->patd[1];
		awt_pro_a_nucs->dist[s]->patd[0] |= 1<<s;
	}

	for (s = 0; s< awt_pro_a_nucs->max_aa; s++){
		sum = 0;
		for (d = 0; d< awt_pro_a_nucs->realmax_aa; d++){
			if ( (1 << d) & awt_pro_a_nucs->dist[s]->patd[1] ){
			        sum |= awt_pro_a_nucs->dist[d]->patd[1];
			}
		}
		awt_pro_a_nucs->dist[s]->patd[2] |= sum;
	}
