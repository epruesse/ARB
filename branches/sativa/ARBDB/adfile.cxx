...
const char *GB_get_arb_revision_tag() {
    /*! retrieve arb revision tag (e.g. "14358@trunk") from ../lib/revision_info.txt
     * File is generated by ../SOURCE_TOOLS/build_info.pl
     */
    static SmartCharPtr tag;

    if (tag.isNull()) {
        char *name = GB_lib_file(true, NULL, "revision_info.txt");
        tag        = GB_read_file(name);

        char *nl    = strchr(&*tag, '\n');
        if (nl) *nl = 0;

        if (tag.isNull()) tag = GBS_global_string_copy("<failed to read %s>", name);
        free(name);
    }
    return &*tag;
}
