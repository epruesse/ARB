...
BOOL BuildHuffmanCode(struct HuffCode *hcbase, ULONG len, LONG threshold) {
    /* generate huffman tree. I know this is not the fastest
     ...
     this should not be a problem (C.Hodges) */
    ...
    struct HuffCodeInternal *hc = (struct HuffCodeInternal *) calloc(
            newlen << 1, sizeof(struct HuffCodeInternal));
    if (!hc) {
    ...
    ULONG min0idx, min0val, min1idx, min1val;
    do {
...
void WriteHuffmanTree(struct HuffCode *hc, ULONG size, FILE *fh) {
    ...
    for (cnt = 0; cnt < size; cnt++) {
        if (hc[cnt].hc_CodeLength) {
            ...
            fwrite(&hc[cnt].hc_CodeLength, sizeof(hc[cnt].hc_CodeLength), 1,
                    fh);
...
struct HuffTree * ReadHuffmanTree(FILE *fh) {
    struct HuffTree *root = (struct HuffTree *) calloc(sizeof(struct HuffTree),
            1);
    if (!root) {
    ...
    do {
        ...
        while (depth++ < codelen) {
            ...
            if (!ht->ht_Child[leafbit]) {
                if (!(ht->ht_Child[leafbit] = (struct HuffTree *) calloc(
                        sizeof(struct HuffTree), 1))) {
                    return (NULL); /* out of memory */
                }
            }
        ...
        {
            printf(
                    "Huffman tree does not comply to the fano condition (%ld: %08lx, %d)!\n",
                    cnt, codec, codelen);
        }
    } while (TRUE);
    return (root);
...
/* /// "FindHuffTreeID()" */
struct HuffTree * FindHuffTreeID(struct HuffTree *ht, UBYTE *adr,
        ULONG bitpos) {
    adr += bitpos >> 3;
    bitpos &= 7;
