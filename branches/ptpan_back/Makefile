...
PT_DEP_COMMON=$(ARBDB_LIB) $(ARCHS_CLIENT_PROBE) PROBE_COM/server.a $(ARCHS_PT_SERVERCOMMON)
PTLINK_COMMON=$(LIBPATH) $(PT_DEP_COMMON) $(SYSLIBS) $(EXECLIBS)

$(PTSERVER): $(subst .a,.dummy,$(ARCHS_PTSERVER_ONLY) $(ARCHS_PT_SERVERCOMMON)) link_db
	@SOURCE_TOOLS/binuptodate.pl $@ $(ARCHS_PTSERVER_ONLY) $(PT_DEP_COMMON) || ( \
		echo "$(SEP) Link $@"; \
...
$(PTPAN): $(subst .a,.dummy,$(ARCHS_PTPAN_ONLY) $(ARCHS_PT_SERVERCOMMON)) link_db
	@SOURCE_TOOLS/binuptodate.pl $@ $(ARCHS_PTPAN_ONLY) $(PT_DEP_COMMON) || ( \
		echo "$(SEP) Link $@"; \
