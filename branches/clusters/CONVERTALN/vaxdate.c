...
/* ####
   ...
   *sun_today_date()   {
   ...
   struct timeval  tp;
   struct timezone tzp;
   char    line[SIZE], *asctime(), *return_value, *Dupstr();
...
/* ---------------------------------------------------------------- */
/*  Function today_date().
    /*      Get today's date.  (VAX/VMS version)
*/
char
*today_date()   {

    int     time_val;
    char        line[SIZE], *ctime(), *return_value, *Dupstr();

    time(&time_val);

    Cpystr(line, ctime(&time_val));
...
*gcg_date(date_string)
     char    *date_string;
{
    char    date[128], temp[128], time[128];
    int day, year, Cmpstr();
    void    Catstr();
