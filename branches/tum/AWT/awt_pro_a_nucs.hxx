...
extern struct arb_r2a_struct {
	...
	struct arb_r2a_pro_2_nuc *s2str[256];	// singlecode protein >> dna ...
    	long	*pro_2_bitset;			// 
	char	*nuc_2_bitset;			// dna to 
	char	index_2_spro[64];		// 64 > max_aa
	int	realmax_aa;			// only real aa + stop
	int	max_aa;				// incl. ambigous
	AWT_PDP	*dist[64];			// 64 > max_aa
					// gen. bu awt_pro_a_nucs_gen_dist
	long	transform07[256];	// like dist.patd[1] but
					// for bits 0-7
...
#define AP_PRO_TYPE_AWAR "nt/protein_codon_type"

char *AP_create_dna_to_ap_bases(void);	// create dna 2 nuc_bitset
void awt_pro_a_nucs_convert_init(GBDATA *gb_main); // init awt_pro_a_nucs, uses AP_PRO_TYPE_AWAR !!!
