...
void MG_tree_rename_cb(AW_window *aww,GBDATA *gbd, int tree_nr){
	...
	char tsource[256];
	char tdest[256];
	sprintf(tsource,"tmp/merge%i/tree_name",tree_nr);
	sprintf(tdest,"tmp/merge%i/tree_dest",tree_nr);
	...
	if (!error) {
		...
		GBDATA *gb_tree_data =
			GB_search(gbd,"tree_data",GB_CREATE_CONTAINER);
		GBDATA *gb_tree_name =
...
void MG_tr_delete_cb(AW_window *aww,GBDATA *gbd, long tree_nr){
	GB_ERROR error = 0;
	char tsource[256];
	sprintf(tsource,"tmp/merge%li/tree_name",tree_nr);
	char *source = aww->get_root()->awar(tsource)->read_string();
	GB_begin_transaction(gbd);
	GBDATA *gb_tree_data =
			GB_search(gbd,"tree_data",GB_CREATE_CONTAINER);
...
void MG_transfer_tree(AW_window *aww){
	...
	GBDATA *gb_tree_data1 =
		GB_search(gb_merge,"tree_data",GB_CREATE_CONTAINER);
	GBDATA *gb_tree_data2 =
		GB_search(gb_dest,"tree_data",GB_CREATE_CONTAINER);
	GBDATA *gb_source =
		GB_find(gb_tree_data1,dest,0,down_level);
	GBDATA *gb_dest_tree =
		GB_find(gb_tree_data2,dest,0,down_level);
