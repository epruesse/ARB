...
void inputoptions()
{
  ...
  for (i = 0; i < (chars); i++) {
    if (weight[i] > 0) {
	  ...
	  ((i % bits >= -1 && i % bits < 31 &&
	    ((1L << (i % bits + 1)) & wagner[i / bits]) != 0) || threshold <= 2.0));
    }
...
long *stps;
{
  ...
  for (i = 0; i < (chars); i++) {
    ...
    if ((unsigned long)l < 32 && ((1L << l) & stps[j - 1]) != 0) {
      if ((unsigned long)l < 32 && ((1L << l) & zeroanc[j - 1]) != 0)
	numszero[i] += weight[i];
      else
	numsone[i] += weight[i];
    }
...
node *r;
{
  ...
  if ((unsigned long)dispbit < 32 &&
      ((1L << dispbit) & r->stateone[dispword - 1]) != 0) {
    if ((unsigned long)dispbit < 32 &&
	((1L << dispbit) & r->statezero[dispword - 1]) != 0)
      r->state = '?';
    else
      r->state = '1';
  } else {
