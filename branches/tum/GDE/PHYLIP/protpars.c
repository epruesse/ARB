...
/*
 * version 3.52c. (c) Copyright 1993 by Joseph Felsenstein. Written by Joseph
...
Static long     spp, nonodes, chars, inseed, outgrno, col, datasets, ith, i,
                j, l, jumb, njumble;
/*
 * spp = number of species nonodes = number of nodes in tree chars = number
 * of sites in actual sequences outgrno indicates outgroup
 */
Static boolean  jumble, usertree, weights, thresh, trout, outgropt, printdata,
                progress, treeprint, stepbox, ancseq, mulsets, interleaved,
...
{
	/*
	 * this and the following are do-it-yourself garbage collectors. Make
	 * a new node or pull one off the garbage list
	 */
	if (garbage != NULL) {
		*p = garbage;
...
{
	/*
	 * sets up for each node in the tree the aa set for site m at that
	 * point and counts the changes.  The program spends much of its time
	 * in this PROCEDURE
	 */
	boolean         counted;
	aas             aa;
		...
		for (i = 0; i <= 1; i++) {
			for (aa = ala; (long) aa <= (long) stop; aa = (aas) ((long) aa + 1)) {
				if (((1L << ((long) aa)) & qs[i]) != 0) {
					for (j = i + 1; j <= 2; j++) {
#if 0
						if (translate[(long) aa - (long) ala][j - i] & ~qs[j])
...
void 
add(below, newtip, newfork)
	node           *below, *newtip, *newfork;
{
	/*
	 * inserts the nodes newfork and its left descendant, newtip, to the
	 * tree.  below becomes newfork's right descendant
	 */
...
{
	/*
	 * removes nodes item and its ancestor, fork, from the tree. the new
	 * descendant of fork's ancestor is made to be fork's second
	 * descendant (other than item).  Also returns pointers to the
	 * deleted nodes, item and fork
	 */
	node           *p, *q, *other;
...
{
	/*
	 * traverses a binary tree, calling PROCEDURE fillin at a node's
	 * descendants before calling fillin at the node
	 */
	if (p->tip)
		return;
...
{
	/*
	 * puts tree from ARRAY place in its proper position in ARRAY
	 * bestrees
	 */
	long            i, FORLIM;
...
{
	/*
	 * temporarily adds one fork and one tip to the tree. if the location
	 * where they are added yields greater "likelihood" than other
	 * locations tested up to that time, then keeps that location as
	 * there
	 */
...
void 
addpreorder(p, item, nufork)
	node           *p, *item, *nufork;
{
	/*
	 * traverses a binary tree, calling PROCEDURE tryadd at a node before
	 * calling tryadd at its descendants
	 */
...
{
	/*
	 * evaluates one rearrangement of the tree. if the new tree has
	 * greater "likelihood" than the old one sets success := TRUE and
	 * keeps the new tree. otherwise, restores the old tree
	 */
...
void 
repreorder(p, success)
	node           *p;
	boolean        *success;
{
	/*
	 * traverses a binary tree, calling PROCEDURE tryrearr at a node
	 * before calling tryrearr at its descendants
	 */
	if (p == NULL)
		return;
	tryrearr(p, success);
...
{
	/*
	 * traverses the tree (preorder), finding any local rearrangement
	 * which decreases the number of steps. if traversal succeeds in
	 * increasing the tree's "likelihood", PROCEDURE rearrange runs
	 * traversal again
	 */
...
void 
addelement(p, nextnode, lparens, names)
	node          **p;
...
void 
hyptrav(r, hypset, b1, b2, k, bottom, nothing)
	node           *r;
	sitearray      *hypset;
	long            b1, b2, *k;
	boolean        *bottom;
...
{
	/*
	 * constructs a binary tree from the pointers in treenode. adds each
	 * node at location which yields highest "likelihood" then rearranges
	 * the tree for greatest "likelihood"
	 */
