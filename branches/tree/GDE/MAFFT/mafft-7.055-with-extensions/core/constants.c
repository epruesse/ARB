...
static void ambiguousscore( int *amino_n, int n_dis[26][26] )
{
	int i;
	for( i=0; i<26; i++ )
	{
...
static void calcfreq( int nseq, char **seq, double *datafreq )
{
	...
	for( i=0; i<20; i++ ) if( datafreq[i] < 0.0001 ) datafreq[i] = 0.0001;
...
// ?????
        ...
        if( disp )
        {
            ...
            fprintf( stderr, "\n" );
			fprintf( stderr, "penalty = %d, penalty_ex = %d\n", penalty, penalty_ex );
        }
