...
ED4_returncode ED4_sequence_terminal::draw() {
    ...
    if (ED4_ROOT->protstruct) {
        ED4_species_manager *spec_man = get_parent(ED4_L_SPECIES)->to_species_manager();
        if (!spec_man->flag.is_SAI_manager && ED4_ROOT->aw_root->awar(PFOLD_AWAR_ENABLE)->read_int()) {  // should do a remap
...
ED4_returncode ED4_text_terminal::draw() {
    ...
    if (is_species_name_terminal()) {
        ...
        int     paint_box      = !parent->flag.is_cons_manager && !parent->parent->parent->flag.is_SAI_manager; // do not paint marked-boxes for consensi and SAIs
