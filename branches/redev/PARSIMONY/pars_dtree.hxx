...
class AWT_graphic_parsimony : public AWT_graphic_tree, virtual Noncopyable {
    ...
    virtual void command(AW_device *device, AWT_COMMAND_MODE cmd, AW_MouseButton button, AW_key_mod key_modifier, AW_key_code key_code, char key_char,
                         AW_event_type type, AW_pos x, AW_pos y,
                         const AW_clicked_line *cl, const AW_clicked_text *ct) OVERRIDE;
    virtual void handle_command(AW_device *device, AWT_graphic_event& event) OVERRIDE;
public:
