...
const char *AWT_graphic_tree::show_ruler(AW_device *device, int gc) {
    ...
    if (tree_awar) {
        ...
        AW_click_cd cd(disp_device, 0, (AW_CL)"ruler");
        device->line(gc,
...
public:
    ...
    void test_show_tree(AW_device *device, AP_tree_sort type, int mode) {
        ...
        scaled_font.init(charLimits, 1/disp_device->get_scale());
        scaled_branch_distance = scaled_font.height * 1.0;
        ...
        switch (type) {
            case AP_TREE_NORMAL: {
                DendroSubtreeLimits limits;
                y_pos             = 0.05;
                show_dendrogram(tree_static->get_root_node(), 0, limits);
                list_tree_ruler_y = y_pos + 2.0 * scaled_branch_distance;
                break;
            }
            case AP_TREE_RADIAL: {
        ...
        if (sort_is_tree_style(type)) { // show rulers in tree-style display modes
            show_ruler(device, AWT_GC_CURSOR);
        }
    }
    ...
    void test_print_tree(AW_device_print *print_device, AP_tree_sort type, bool show_handles, int mode) {
        ...
        print_device->set_offset(offset/print_device->get_scale());
