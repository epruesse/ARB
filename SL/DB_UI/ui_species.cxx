...
AW_window *DBUI::create_fields_reorder_window(AW_root *root, AW_CL cl_bound_item_selector) {
    ...
    if (!awsa[selector.type]) {
        ...
        Itemfield_Selection *sel1 = create_selection_list_on_itemfields(bound_selector->gb_main, aws, AWAR_FIELD_REORDER_SOURCE, FIELD_FILTER_NDS, "source", 0, selector, 20, 10, SF_STANDARD, NULL);
        Itemfield_Selection *sel2 = create_selection_list_on_itemfields(bound_selector->gb_main, aws, AWAR_FIELD_REORDER_DEST,   FIELD_FILTER_NDS, "dest",   0, selector, 20, 10, SF_STANDARD, NULL);
...
AW_window *DBUI::create_field_delete_window(AW_root *root, AW_CL cl_bound_item_selector) {
    ...
    if (!awsa[selector.type]) {
        ...
        Itemfield_Selection *item_sel =
            create_selection_list_on_itemfields(bound_selector->gb_main, aws, AWAR_FIELD_DELETE, -1, "source", 0, selector, 20, 10, SF_HIDDEN, NULL);
...
static AW_window *create_field_convert_window(AW_root *root, AW_CL cl_bound_item_selector) {
    ...
    create_selection_list_on_itemfields(bound_selector->gb_main, aws, AWAR_FIELD_CONVERT_SOURCE, -1, "source", 0, selector, 40, 20, SF_HIDDEN, NULL);
...
static AW_window *create_next_neighbours_listed_window(AW_root *aw_root, AW_CL cl_query) {
    ...
    if (!aws) {
        ...
        aws->at("field");
        create_selection_list_on_itemfields(query_get_gb_main((DbQuery*)cl_query), aws, AWAR_NN_LISTED_DEST_FIELD, (1<<GB_INT) | (1<<GB_STRING), "field", 0, SPECIES_get_selector(), 20, 10, SF_STANDARD, NULL);
...
static AW_window *popup_new_speciesOrganismWindow(AW_root *aw_root, GBDATA *gb_main, bool organismWindow, int detach_id) { // INFO_WINDOW_CREATOR
    ...
    aws->at("help");
    aws->callback(makeHelpCallback(detach_id ? "sp_info_locked.hlp" : "sp_info.hlp")); // uses_hlp_res("sp_info_locked.hlp", "sp_info.hlp"); see ../../SOURCE_TOOLS/check_ressources.pl@uses_hlp_res
    aws->create_button("HELP", "Help", "H");
