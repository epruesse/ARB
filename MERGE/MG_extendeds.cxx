...
AW_window *MG_merge_extendeds_cb(AW_root *awr) {
    ...
    DbScanner *scanner = create_db_scanner(GLOBAL_gb_merge, aws, "info1", 0, 0, 0, DB_SCANNER, 0, 0, 0, SPECIES_get_selector());
    aws->get_root()->awar(AWAR_EX_NAME1)->add_callback(MG_map_extended1, (AW_CL)scanner);

    aws->at("ex2");
    awt_create_selection_list_on_extendeds(GLOBAL_gb_dest, (AW_window *)aws, AWAR_EX_NAME2);
    scanner = create_db_scanner(GLOBAL_gb_dest, aws, "info2", 0, 0, 0, DB_SCANNER, 0, 0, 0, SPECIES_get_selector());
    aws->get_root()->awar(AWAR_EX_NAME2)->add_callback(MG_map_extended2, (AW_CL)scanner);
